	...
	int numi)
{
	...
	if (index < numi){			// check to see that threads only work if an image exists
		...
		d_beta1[1] = 0.0;
		d_beta1[2] = 0.0;
		d_beta1[3] = ((double)pix)/4;						 // guess on sigma widths
		d_beta1[4] = ((double)pix)/4;						 // guess on sigma widths
		d_beta1[5] = 100000;
...
// localize 9
__global__ void  localize9(double *d_iall,
	double *d_xf_all,
	double *d_yf_all,
	double *d_N,
	double *d_off,
	...
	int numi)
{
	...
	if (index < numi){			// check to see that threads only work if an image exists
		...
		d_beta1[1] = 0.0;
		d_beta1[2] = 0.0;
		d_beta1[3] = ((double)pix-3)/4;						 // guess on sigma widths
		d_beta1[4] = ((double)pix-3)/4;						 // guess on sigma widths
		d_beta1[5] = 100000;
		for (int i = 0; i <pix*pix; i++) {
			d_i2[i] = d_iall[i + index*pix*pix];	         // this buffers [49] pixels into each d_i2, the thread index determines which image is analyzed
			d_beta1[0] += xgrid[i]*d_i2[i];						 // sum of x and image weight
...
// localize 11
__global__ void  localize11(double *d_iall,
	double *d_xf_all,
	double *d_yf_all,
	double *d_N,
	double *d_off,
