%% Mito analyze
% A script to help Jaime analyze mitochondria
%
%
% This is a work in progress that I intend to update as I move along, but
% for now the general idea is that Jaime will hand me 2 data sets, one with
% a mito signal that shows where mitochondria are, the other with a
% potentially contaminated signal, which we want to find our response in. I
% can start by importing ROIs from image J, cutting out the resulting
% cell's ROI, then identifying regions for the mitochondria, and looking at
% the corresponding regions in the response channel for the appropriate
% response
% AJN 1-23-2018 Ryan Lab
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clearvars; close all; clc;

%% USER VARIABLES
% This code is the cave of wonders, and this section is the lamp
% You are Aladdin, touch nothing but the lamp!

thresh_percent = 10;  % Choose the threshold value as a percentage of the 'maximal pixel'
       stim_on = 50;  % frame on which stimulation occurs
      back_pix = 10;  % pixel radius for determining background

%% Setup and loading
% Grab ROI and image file information
finfo = dir('*.roi');
imfo = dir('*.tif');

% check filename for 'green' and assign images to green / red channel
if strfind(imfo(1).name,'green')
    ig = readtiff(imfo(1).name);
    ir = readtiff(imfo(2).name);
else
    ir = readtiff(imfo(1).name);
    ig = readtiff(imfo(2).name);
end

% load roi information
for i = 1:numel(finfo)
    s(i) = getimjroi(finfo(i).name);
end
%% at this point all the relevant data should be acquired
[m,n,o] = size(ir);


% Get measurement of background for thresholding
imagesc(ir(:,:,1))
title('Red Channel');
xlabel('choose a region for background');
try
    while true
        [x,y] = ginput(1);
        imagesc(ir(:,:,1))
        draw_boxes(1,[x,y, -2], back_pix);
        title('Exit to continue or select new region');
    end
catch lsterr
end
% Select the draw boxes ROI
x0=x-back_pix;
y0=y-back_pix;
x1=x+back_pix;
y1=y+back_pix;
si1 = ir(round(y0:y1),round(x0:x1), :);  % extract sub red image
si2 = ig(round(y0:y1),round(x0:x1), :);  % extract sub green image
rbkgn = mean(si1(:));  % takes the mean value of the ROI over all frames
gbkgn = mean(si2(:));  % takes the mean value of the ROI over all frames


stim = round(stim_on/2);
for i = 1:numel(s) % loop over all ROIs
    % The nature of this data is going to be such that we'll want to store
    % everything in a structure variable
    a = s(i).vnRectBounds;
    % cut the region out
    sig = ig(a(1):a(3),a(2):a(4),:);
    sir = ir(a(1):a(3),a(2):a(4),:);
    
    % region ID
    % Send the roi throuhg imPCA
%     sip = ImsPCA(sig,3);
    % backsubtract this
    ip = rollingball(sig);
    % all frames are now ipd and bkgd
    for k = 1:o % loop over all frames and get the mask
        % set threshold to be 10% the maximum value
        ips = ip(:,:,k);
        thresh = thresh_percent/100*max(ips(:));
        mask(:,:,k) = single(ips >= thresh);
        nmask(:,:,k) = mask(:,:,k)/sum(sum(mask(:,:,k)));
        mask2(:,:,k) = single(sir(:,:,k) >= 0.1*max(sir(:)));
    end
        rsig = nmask.*sir; % create mask for red signal
        fulr = mask2.*sir; % create the total image in the channel
        gsig = nmask.*sig; % create masked signal in green
        rest = sum(sum(logical(1-mask2).*sir))./sum(sum(logical(1-mask2)));  % Sub image containing non-cell pixels
        restg = sum(sum(logical(1-mask2).*sig))./sum(sum(logical(1-mask2))); % same as above except for the green channel
        Fr = sum(sum(rsig)); % F trace in Red
        mper = sum(sum(mask.*sir))./sum(sum(fulr));  % The percentage of 'response' pixels to all pixels in the 'cell'
        rchck = sum(sum(rsig))/rbkgn;       % The check that the mitos is brighter than the surrounding area
        gchck = sum(sum(gsig))/gbkgn;       % The check that the mitos is brighter than the surrounding area
        Fg = sum(sum(gsig)); % raw F trace in green
        rest = rest(:);   % Trace of the average non-cell pixel in red channel
        restg = restg(:); % Trace of the average non-cell pixel in green channel
        Fr = Fr(:) - rbkgn;  % subtract background from Red channel
        Fg = Fg(:) - gbkgn; % subtract background from green channel
        nfr = mean(Fr(2:stim-2)); % determine baseline value (nfr for normalize fluor. red)
        nfg = mean(Fg(2:stim-2)); % determine baseline value in green
        dFr = (Fr - nfr)./nfr;    % subtract off baseline and normalize the curve red
        dFg = (Fg - nfg)./nfg;    % subtract off baseline and normalize the curve green
        mper = mper(:);           % get the fluorescent pixel percentage response
        rchck = rchck(:);           % get the cell/non-cell percentage
        gchck = gchck(:);
        % Assign variables to structure variable for convenience
        fin(i).mask = nmask(:,:,2:end-1);      % mito mask normalized (should sum to 1)
        fin(i).amask = mask2(:,:,2:end-1);     % mask of cell fluorescence
        fin(i).green = sig(:,:,2:end-1);       % total green channel measured
        fin(i).red = sir(:,:,2:end-1);         % total red channel measured
        fin(i).rsig = rsig(:,:,2:end-1);       % Total red signal channel measured
        fin(i).gsig = gsig(:,:,2:end-1);       % Total Green signal channel measured
        fin(i).Fr = Fr(2:end);                 % background subtracted F in green
        fin(i).Fg = Fg(2:end);                 % background subtracted F in green
        fin(i).dFr = dFr(2:end-1);             % dF in red channel
        fin(i).dFg = dFg(2:end-1);             % dF in green channel
        fin(i).mper = mean(mper(2:stim-2));    % Average Mito/non-mito fluorescent response over frames
        fin(i).rcheck = mean(rchck(2:stim-2));    % Average cell/non-cell response over frames
        fin(i).gcheck = mean(gchck(2:stim-2));
        fin(i).fulr = fulr;                    % Full cell fluorescence in red channel
        fin(i).a = a;                          % Image coords of ROI
        clear mask sip sir sig a thresh ips gsig rsig Fr Fg dFr dFg mper mask2 fulr nmask
end
save([imfo(1).name(1:end-4),'_mito.mat'],'fin')
% Data Representation
figure
subplot(1,2,1);
Tr = 0*fin(1).dFr;
...
data =[];
for i = 1:numel(fin)
    data = [data,[fin(i).mper;fin(i).rcheck;fin(i).dFr],[fin(i).mper;fin(i).gcheck;fin(i).dFg]];
end
xlswrite('your_excel.xls',data);

%% END USEFUL EDITING PORTION
% EVERYTHING BELOW THIS LINE WORKS AND SHOULD BE LEFT ALONE 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% debugging/visualization stuff
% bl = 3;
% subplot(2,2,1);
% imagesc(fin(bl).fulr(:,:,20));
% axis image
% subplot(2,2,2);
% imagesc(fin(bl).rsig(:,:,20));
% axis image
% subplot(2,2,3);
% imagesc(fin(bl).red(:,:,20));
% axis image
% subplot(2,2,4);
% plot(fin(bl).mper)
...
function [sROI] = getimjroi(cstrFilenames)
