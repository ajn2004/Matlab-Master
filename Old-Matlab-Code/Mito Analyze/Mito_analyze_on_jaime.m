...
for i = 1:numel(s) % loop over all ROIs
    ...
    for k = 1:o % loop over all frames and get the mask
        ...
        rest = logical(1-mask2).*sir;
        Fr = sum(sum(rsig)); % F trace in Red
        mper = sum(sum(mask.*sir))./sum(sum(fulr));
        chck = sum(sum(rest))./sum(sum(fulr));
        Fg = sum(sum(gsig)); % F trace in green
        nfr = mean(sum(sum(rsig(:,:,2:stim-2))));
        nfg = mean(sum(sum(gsig(:,:,2:stim-2))));
        Fr = Fr(:);
        Fg = Fg(:);
        dFr = (Fr - nfr)./nfr;
        dFg = (Fg - nfg)./nfg;
        ...
        fin(i).mask = nmask(:,:,2:end-1);
        fin(i).green = sig(:,:,2:end-1);
        fin(i).red = sir(:,:,2:end-1);
        fin(i).rsig = rsig(:,:,2:end-1);
        fin(i).gsig = gsig(:,:,2:end-1);
        fin(i).Fr = Fr(2:end);
        fin(i).Fg = Fg(2:end);
        fin(i).dFr = dFr(2:end-1);
        fin(i).dFg = dFg(2:end-1);
        fin(i).mper = mean(mper(2:stim-2));
        fin(i).chck = mean(chck(2:stim-2));
        fin(i).fulr = fulr;
        fin(i).a = a;
...
data =[];
for i = 1:numel(fin)
    data = [data,fin(i).Fr,fin(i).dFr,fin(i).Fg,fin(i).dFg];
end
