...
[fname1, fpath1] = uigetfile('*traj.mat','Choose a file to analyze');
load(strcat(fpath1,fname1));
...
corr_angs(corr_angs <= -pi) = corr_angs(corr_angs <= -pi)+2*pi;


fig2 = figure ; 
deg_ang = corr_angs*360/(2*pi);
hist(corr_angs.*360./(2*pi),-180:10:180);
xlabel('Angle in Degrees from best fit');
ylabel('Frequency in Number');
saveas(fig2, strcat(fpath1,fname1(1:numel(fname1)-4),'_angle_distribution'),'fig');
saveas(fig2, strcat(fpath1,fname1(1:numel(fname1)-4),'_angle_distribution'),'tif');