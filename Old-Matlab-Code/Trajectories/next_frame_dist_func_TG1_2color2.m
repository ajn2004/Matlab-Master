function [q,data_all,xf_all,yf_all,framenum_all,a0_all,total_molecules,nrat,green_sum,red_sum]=next_frame_dist_func_TG1_2color2(filename,microns_per_pixel,dmax,dfactor,dfactor2,mol_type)

%mol_type=1;

try
    load(filename,'q','xf_all','yf_all','framenum_all','nrat','green_sum','red_sum');
    oh = nrat;
catch err
    load(filename,'q','xf_all','yf_all','framenum_all');
    nrat = 0.5*ones(numel(xf_all),1);
    green_sum = 51*ones(numel(xf_all),1);
    red_sum = 51*ones(numel(xf_all),1);
end

xf_all0=xf_all;
yf_all0=yf_all;
framenum_all0=framenum_all;
total_molecules = numel(xf_all);
