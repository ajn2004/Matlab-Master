	...
	int icol)
{
	...
	if ((row_input >= 0) && (row_input < irow) && (col_input >= 0) && (col_input < icol)) {		// if statement checks the row/col indices to ensure they fall onto the input image
		d_xs[ty][tx] = d_x[row_input*icol + col_input];										// if true, the value of the image is written to the shared array at location d_i2[ty][tx] and stored locally
	}																							// on the block
	else {
	...
	if (ty < O_TILE_WIDTH && tx < O_TILE_WIDTH) {
		for (int i = 0; i < v; i++) {
			for (int j = 0; j < v; j++) {
				...
				d_y[(i*v +j)*irow*icol+row_output*icol+col_output] = d_xs[ty+i][tx+j];
}
			}
		}
	}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, mxArray const *prhs[])
{
	...
	const int w = mxGetScalar(prhs[1]);
	const int v = 2 * w + 1;
	...
	imswitch << <dimGrid, dimBlock >> > (dx, dy, w, m, n);
	printf("Before Pointer Manipluation\n");
	plhs[0] = mxCreateDoubleMatrix(m*n, v*v, mxREAL);
	oy = mxGetPr(plhs[0]);
