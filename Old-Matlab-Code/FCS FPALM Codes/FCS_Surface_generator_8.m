...
int_col = 1; % This is the column from the excel data that the intensity is located
count_col = 2; % This is the column from the excel data that the adjusted counts / molecule/sec are located
bkgn_col = 7; % This is the column from the excel data that the avg background counts are located
flick_col = 6; % colomn in data where the flicker fraction occurs
...
det_prob = .5; % detection efficiency
...
hold on
contour(intensities,xax,lu.',50); % contour map
set(gca,'XScale','log');
ylabel('Frame exposure time in s');
xlabel('Intensity in kW/cm^2');
zlabel('Localization Uncertainty in nm')
