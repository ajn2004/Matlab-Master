...
an_dir = 'F:\Neural Learning\';   %Correlation file needs to be in this same folder
% an_dir = 0;
...
pix2pho = 40;          % Pixel to photon ratio (for sCMOS set to 1 after pre-processing)
...
addpath(fpath);
ninfo = dir(['Neural_thetas_it_*', num2str(it),'*']);
...
%% Single Color Batching
    ...
    for j=file_start:file_end
        gauss_scale = 1;
        bkgn = start_bkgns(j);
        base_name=File_Info(j).name;
        %           base_name=base_name(1:end-4);
        tic
        func_Quhzx_02_5(data_dir,base_name,an_dir,bkgn,Theta1, Theta2,q,pix2pho,n_start,n_end, wvlngth, NA, n_bkgn);
        toc
    end
