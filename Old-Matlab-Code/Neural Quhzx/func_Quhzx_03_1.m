function func_Quhzx_03_1(fpath,fname,an_dir,bkgn,Theta1, Theta2, q,pix2pho,n_start,n_end, wvlngth, NA, n_bkgn, corr_name)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% func_Quhzx v 3.01
%
...
% v 3.01 setup to handle multicolor information
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% load correlation file
corr_file = [data_dir, corr_name];
load(corr_file);
load(corr_file,'alpha','beta');
Ax1=Acoords(1);                     % define transform parameters
Ax2=Acoords(2);
Ay1=Acoords(3);
Ay2=Acoords(4);
Bx1=Bcoords(1);
...
if n_end == 0  % if end frame is 0 set end frame to final frame
    n_end = numel(imagfo);
end
n_end

% preallocate space and load image into workspace
...
% cut out relevant areas
A=iprod(Ay1:Ay2,Ax1:Ax2,:);
B=iprod(By1:By2,Bx1:Bx2,:);

% perform interpolation over all frames
for i = 1:numel(iprod(1,1,:))
    Z2(:,:,i)=interp2(X,Y,B(:,:,i),X2,Y2)/(alpha*beta);
end
Z2(isnan(Z2))=0; % set any 'NaN' to zero
clear iprod
iprod=A+Z2; % add up all areas to create final frame
clear B
...
for i = 1:num_chunk  %This loop system is set up to deal with images so that they don't overload the gpu
    if i~= num_chunk
...
%% Frame Analysis
fprintf('Localizing, please wait');
...
for framenum = n_start:n_end                               % start on frame n_start and cycle through frame n_end
    if framenum/100 == round(framenum/100)
        fprintf('.');
        dots = dots +1;
        if linees == 0;
            if dots ==60
    ...
    [mol_locs, num_mol] = neural_gpu(a1(:,:,framenum-n_start + 1));   % Perform neural net calculation
    ...
    if 1 - isempty(xpeak);                  % thisif statement will skip blank frames
        ...
        if numel(xpeak)  >0
            for pm = 1:numel(xpeak)
                ...
                beta0(5,mol_num) = sigma;
                beta0(6,mol_num) = off_guess;
                xpeaks(mol_num,1) = xpeak(pm);
                ypeaks(mol_num,1) = ypeak(pm);
                framenum_temp(mol_num,1) = framenum;
                i4(:,:,mol_num) = i3(:,:);
                im_green = A(round(yc_box)-rbox2:round(yc_box)+rbox2,round(xc_box)-rbox2:round(xc_box)+rbox2,framenum);
                im_red= Z2(round(yc_box)-rbox2:round(yc_box)+rbox2,round(xc_box)-rbox2:round(xc_box)+rbox2,framenum);
                green_sum_temp(mol_num,1) = sum(im_green(:));
                red_sum_temp(mol_num,1) = sum(im_red(:));
                clear i3;
                ...
                if mol_num == 100000
                    ...
                    if  runs ==1
                        ...
                        green_sum = green_sum_temp(framenum_all_temp >0);
                        red_sum = red_sum_temp(framenum_all_temp >0);
                        ...
                        green_sum = vertcat(green_sum,green_sum_temp(framenum_all_temp >0));
                        red_sum = vertcat(red_sum,red_sum_temp(framenum_all_temp >0));
...
save([an_dir,fname(1:end-4),'_frames_',num2str(n_start),'_',num2str(n_end),'_t_neuro'],'nrat','ratio','green_sum','red_sum','xf_all','yf_all','N','off_all','sigx_all','sigy_all','sigx_crlb','sigy_crlb', 'xf_crlb','yf_crlb','N_crlb', 'off_crlb', 'sigma2','wvlngth','q','pix2pho','NA','n_start','n_end','n_bkgn','bkgn','Theta1','Theta2','llv','framenum_all','lp2','lp');
end
