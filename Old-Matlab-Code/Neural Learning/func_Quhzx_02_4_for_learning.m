function [savename] = func_Quhzx_02_4_for_learning(fpath,fname,an_dir,bkgn,Theta1, Theta2, q,pix2pho,n_start,n_end, wvlngth, NA)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% func_Quhzx v 2.04
%
...
for i = n_start:n_end
    j = i-n_start +1;
    i1(:,:,j) = double(imread([fpath,fname],framelist(i),'Info',imagfo))/pix2pho;
end
disp('done loading');
% disp('Past Loading');
% tic
...
for framenum = n_start:n_end                               % start on frame n_start and cycle through frame n_end
    ...
    if 1 - isempty(xpeak);                  % thisif statement will skip blank frames
        if framenum == look
            subplot(4,4,[1,2,5,6,9,10,13,14]); imagesc(i2);
            colormap('gray')
            axis image
...
save([an_dir,savename],'yc','xc','i5','i6','xf_all','yf_all','N','off_all', 'xf_crlb','yf_crlb','N_crlb', 'off_crlb', 'sigma2','wvlngth','q','pix2pho','NA','n_start','n_end','bkgn','Theta1','Theta2','llv','framenum_all');
end
