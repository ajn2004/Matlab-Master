function [savename] = func_Quhzx_02_5_for_learning(fpath,fname,an_dir,bkgn,Theta1, Theta2, q,pix2pho,n_start,n_end, wvlngth, NA, amem, sp)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% func_Quhzx v 2.04
%
...
wgrid = 2*ceil(sigma2)+1;                                                                   % width of the grids for fitting
...
i1t = readtiff([fpath,fname])/pix2pho;
i1 = i1t(:,:,framelist(1:n_end-n_start+1));
disp('done loading');
% disp('Past Loading');
% tic
...
max_neur = 200*150*500*8; %this number is based off simulations
a1 =[];
...
for i = 1:num_chunk
    if i~= num_chunk
        [atemp] = image_neural_norm(double(iprod(:,:,1+ (i-1)*chunk_size: i*chunk_size)), Theta1.', Theta2.', chunk_size);
        if i ==1
            a1 = atemp;
            clear atemp
        else
        ...
        [atemp] = image_neural_norm(double(iprod(:,:,1+ (i-1)*chunk_size:end)), Theta1.', Theta2.', numel(iprod(1,1,1+ (i-1)*chunk_size:end)));
        a1 = cat(3,a1,atemp);
        clear atemp
    end
...
for framenum = 1:n_end-n_start+1                               % start on frame n_start and cycle through frame n_end
