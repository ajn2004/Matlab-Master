function [savename] = func_Quhzx_02_5_for_learning(fpath,fname,an_dir,bkgn,Theta1, Theta2, q,pix2pho,n_start,n_end, wvlngth, NA, amem)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% func_Quhzx v 2.04
%
...
% v 2.01 Interfacing with GPU has been improved to handle variable imagez
...
framelist = randperm(numel(imagfo));

i1 = zeros(imagfo(1).Height,imagfo(1).Width,n_end-n_start+1);

tlink = Tiff([fpath,fname],'r');
for i = n_start:n_end
    j = i-n_start +1;
    tlink.setDirectory(framelist(j));
    i1(:,:,j) = double(tlink.read())/pix2pho;
end
i1(:,:,j+1) = double(imread([fpath,fname],60)/pix2pho);
disp('done loading');
% disp('Past Loading');
% tic
...
disp('Done Subtracting and Neural calc');
...
for i = 1:num_chunk
    if i~= num_chunk
        [atemp] = image_neural_3(iprod(:,:,1+ (i-1)*chunk_size: i*chunk_size), Theta1.', Theta2.', chunk_size);
        if i ==1
            a1 = atemp;
            clear atemp
        else
...
for framenum = n_start:n_end+1                               % start on frame n_start and cycle through frame n_end
    ...
    [mol_locs, num_mol] = neural_gpu(a1(:,:,framenum));   % Perform neural net calculation
    ...
    if 1 - isempty(xpeak)                  % thisif statement will skip blank frames
        ...
        if numel(xpeak)  >0
            for pm = 1:numel(xpeak)
                ...
                xguess = (sum(sum(xgrid.*i3))/(sum(sum(i3))));                                % initial value for the x position
                yguess = (sum(sum(ygrid.*i3))/(sum(sum(i3))));                                % initial value for the y position
...
%                         i5 = i4((numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 ,(numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 , framenum_all_temp>0);
%                         i6 = i4((numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 ,(numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 , framenum_all_temp<0);
                        ...
                        disp([num2str(100*framenum/(1+n_end-n_start)),'% complete']);
                        runs = runs+1;
                        %                         i5 = cat(3,i5,i4((numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 ,(numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 , framenum_all_temp>0));
                        %                         i6 = cat(3,i6,i4((numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 ,(numel(i4(:,1,1))+1)/2 - 3 :(numel(i4(:,1,1))+1)/2 + 3 , framenum_all_temp>0));
...
clear num_chunk num_mol sigma sigma2 sigma2_um tlink sigx_crlb_temp sigy_crlb_temp sigx_temp sigy_temp
