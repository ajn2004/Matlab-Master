...
% grid_size = 30; % in nm
% 
% [fname, fpath] = uigetfile('*.mat','Select a localization file');  % grab file to be analyzed
% cd(fpath);
% load([fpath,fname]); %load desired file
% 
% %% Show the data
% f1 = figure;
% screen_size = get(0, 'ScreenSize');
% set(f1, 'Position', [0 0 screen_size(3) screen_size(4) ] );
% h1=plot(xf_all*q,yf_all*q, '.b');
% axis equal
...
% threshold = 5;
% [row, col] = find(dens < threshold);   %find molecules below threshold
% filt_dens = dens;
% 
...
try
    while true %repeat until user chooses to move on
        ...
        if s == 'y' || s == 'Y'
            ...
            [row, col] = find(i2filt < threshold);
            i2filt = i2;
            for j = 1:numel(row)
                i2filt(row(j),col(j)) = 0;
            end
            dens_filt = medfilt2(i2filt);
            [row, col] = find(dens_filt < 0.5*threshold);
            for j = 1:numel(row)
...
bits_Fin = i2filt.*0; % allocate array
...
try
    while true
        ...
        disp(['Your threshold area of ', num2str(area_thresh),' pixels^2 corresponds to ', num2str(area_thresh*grid_size^2/1000000),'um^2']);
