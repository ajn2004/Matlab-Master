...
% this function returns Z in microns from sigma widths in pixels
    x_spread = z_cal.zlims(1):0.001:z_cal.zlims(2); % Curve span in microns
%     x_spread = -1:0.001:1;
...
%     D = (((sigma_x(i).^0.5-x_curve.^0.5).^2 + (sigma_y(i).^0.5-y_curve.^0.5).^2)*wght + (1-wght)*((sigma_y(i)/sigma_x(i)-y_curve./x_curve).^2)).^0.5;
%     D = (((sigma_x(i).^0.5-x_curve.^0.5).^2 + (sigma_y(i).^0.5-y_curve.^0.5).^2)*wght + (1-wght)*((sigma_y(i)-sigma_x(i))-(y_curve-x_curve))).^0.5;
%     D = abs(x_curve./y_curve - sigma_x(i)/sigma_y(i));
    ind = find(D == min(D), 1);
    min_d(i) = D(ind);
    if D(ind) < 0.4 && (ind ~= 1 || ind ~= numel(x_spread))
%         plot(x_spread,D)
%         hold on
        try
