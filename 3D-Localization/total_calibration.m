...
for i = 1:numel(files)                                                      % Loop over all files
    ...
    ind = col < 200; % Remove 'red' molecules
    row(ind) = [];
    col(ind) = [];
...
for i = 1:numel(psfs) % loop over all psfs
    if i ~= 1 && i ~= 6
        ...
        if abs(dff(i)) < 40 % If displacement is within a 'range', keep for further analysis
            fnumc(cid) = fnumc(cid) - dff(i);
...
smooth_sx = gausssmooth(ssx,2,10);                                                            % Plot average sigma-x
smooth_sy = gausssmooth(ssy,2,10);
plot(ax,z0s,smooth_sx)
...
new_z_cal.sy = smooth_sy;
new_z_cal.z0s = z0s;
zf_um = get_spline_z(sx,sy,new_z_cal);
hold off
xlim(ax,[-1.2,1.2]);
...
% a = polyfit(z0m,zfm,1);
ind = zc < 0 & zf_um > -0.73; % Parameter for inclusiary data for linear fit
a = polyfit(zc(ind),zf_um(ind),1);
% clear zfm
...
% Build a k-means parameter model of the curve
[model] = get_z_tilt_parameters(data, 50);

% Remove clsuter points on a distance basis from an exclusionary point
outlier = [0.37218, -0.54018, 0.72279]; % This parameter is near the outlier cluster point(s) to be excluded from analysis
D = model(:,1)*0;
for i = 1:3
    D = D + (model(:,i) - outlier(i)).^2;
end
D = D.^0.5;
sort(D)
removal_ind = D < 0.75;
...
% Smooth model with a gaussian for subsequent spline fitting
[dz, I] = sort(model(:,3));
model = model(I,:);
splz = min(model(:,3)):0.01:max(model(:,3));

for i = 1:3
    model(:,i) = gausssmooth(model(:,i),2.5,8);
end

% Assign axial tilt parameters to z_calibration
new_z_cal.x_tilt = model(:,1);
new_z_cal.y_tilt = model(:,2);
new_z_cal.z_tilt = model(:,3);

% Get tilt parameters through spline interpolation
[splx, sply] = get_axial_tilt_spline(new_z_cal,splz);


ts = uitab(tg5,'Title','Z-Tilt w/ Spline');
ax = axes(ts);
plot3(ax,model(:,1),model(:,2),model(:,3),'r.','MarkerSize',20)
hold on
plot3(ax,xt,yt,zt,'.')
plot3(ax,splx,sply,splz)
hold off
xlabel('Lat-X');
ylabel('Lat-Y');
zlabel('Axi-Z');
legend('K Means Pts','Spline Curve');

% Make Drift Correction
zf = zf_um/q;
% xc = spline(zs(1:end-1) + mean(diff(zs))/2,xtilt,zf);
% yc = spline(zs(1:end-1) + mean(diff(zs))/2,ytilt,zf);
[xc, yc] = get_axial_tilt_spline(new_z_cal,zf);
...
ax = axes(tc);
% scatter3(ax,xf_c(indy)*q,q*yf_c(indy),q*zf_c(indy)/a(1),[],psf(indy));
new_z_cal.a = a;
ncoords = make_astigmatism_corrections([xt,yt,zt],new_z_cal,q);
plot3(ax,ncoords(:,1),ncoords(:,2),ncoords(:,3),'.')
colormap('jet')
xlabel('Lat-X');
ylabel('Lat-Y');
zlabel('Axi-Z');
