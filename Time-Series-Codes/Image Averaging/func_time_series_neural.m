function [fluor, points, xs] = func_time_series_neural(ifin, peak, ave2,peakresponse, psize, baseline)
close all
...
if flag ==0
    ...
    if numel(points) > 0
        ...
        axis image
        waitforbuttonpress
        sy = input('Would you like to pick different points?(y/n)', 's');
        hold off
    else
        sy = 'y';
    end
    if strcmp(sy,'Y') || strcmp(sy,'y')
        pl = gca;
        clear points
        hold on
        while true
