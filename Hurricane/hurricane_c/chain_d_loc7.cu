...
// localize 7 *updating for 3D 2/7/18
__global__ void  localize7(float *d_iall,  // pointer to our image variable
	...
	int numi)// pointer for final x-coordinate measurement
{
	...
	if (index < numi) {			// check to see that threads only work if an image exists
		...
		if (xf == xf && yf == yf && zf == zf && N == N && b == b && sx == sx && sy == sy && b == b) { // begin is numeric if statement
			if (N > 0 && xf >= -2 && xf <= 2 && yf <= 2 && yf >= -2) { // was the molecule inside the image? Was N positive? if yes then record the point
	...
	int nrhs, mxArray const *prhs[])
{
	...
	localize7 << <((numi - 1) / threadsperblock + 1), threadsperblock >> >(d_iall, d_xf_all, d_yf_all, d_zf_all, d_N, d_off, d_xf_crlb, d_yf_crlb, d_zf_crlb, d_N_crlb, d_off_crlb, d_llv, angle, d_zcurve, numi);
