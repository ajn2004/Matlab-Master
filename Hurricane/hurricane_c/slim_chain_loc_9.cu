...
// localize 7
__global__ void  localize(float *d_iall,
	float *d_xf_all,
	float *d_yf_all,
	float *d_N,
	float *d_off,
	...
	int numi)
{
	...
	// Declare variables
	const int pix = 9;   // number of pixels in the localization image
	__shared__ float xgrid[pix*pix];			// allocate xpix and ypix variables to the shared memory of the blocks
	__shared__ float ygrid[pix*pix];			// this will reduce calls to global device memory
	...
	float d_i2[pix];				// initialize data for image
	float llv;
	...
	if (index < numi){			// check to see that threads only work if an image exists
		...
		d_beta1[1] = 0.0;
		d_beta1[2] = 0.0;
		d_beta1[3] = ((float)pix)/4;						 // guess on sigma widths
		d_beta1[4] = ((float)pix)/4;						 // guess on sigma widths
		d_beta1[5] = 100000;
		...
		if (d_beta1[0] == d_beta1[0] && d_beta1[1] == d_beta1[1] && d_beta1[2] == d_beta1[2] && d_beta1[5] == d_beta1[5] && d_beta1[3] == d_beta1[3] && d_beta1[4] == d_beta1[4] && d_beta1[5] == d_beta1[5]){ // begin is numeric if statement
			if (d_beta1[2] > 0 && d_beta1[0] >= -pix / 2 && d_beta1[0] < pix/2 && d_beta1[1] < pix / 2 && d_beta1[1] >= -pix / 2 && d_beta1[3] > 0 && d_beta1[3] < 10 && d_beta1[4] < 10){ // was the molecule inside the grid? Was N positive? if yes then record the point
...
{	
	...
	localize << <((numi - 1) / threadsperblock + 1), threadsperblock >> >(d_iall, d_xf_all, d_yf_all, d_N, d_off, d_sigx_all, d_sigy_all, d_xf_crlb, d_yf_crlb, d_N_crlb, d_off_crlb, d_sigx_crlb, d_sigy_crlb, d_llv, angle, numi);
