function [im1, um, rad] = make_3dpics(fdname, varargin)
...
zf_all = func_shift_correct(zf_all,framenumber,1);
...
maxz = ceil(max(zf_all)); 
minz = floor(min(zf_all));
dz = dz/(q*1000);
...
for i = 1:numel(xf_all) % loop over each molecule
    ...
    % color selection
    mz = (z_ind - zf_all(i)).^2;  % squared difference between color map and molecule
