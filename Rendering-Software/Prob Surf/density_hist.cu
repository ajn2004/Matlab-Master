...
void mexFunction(int64_t nlhs, mxArray *plhs[],
    int64_t nrhs, mxArray const *prhs[])
    {
        ...
        float *d_i1, *d_i2, *d_iy, *d_iz, *d_x, *d_y, *d_z;
        ...
        populate <<<dimGrid,dimBlock>>> (d_x, d_y, d_z, d_i1, imsize[0], imsize[1], imsize[2], bigly[0], radius);
        CUDA_CHECK_ERROR();
        
        laplace <<<dimGrid,dimBlock>>> (d_i1, d_i2, imsize[0], imsize[1], imsize[2]);
        CUDA_CHECK_ERROR();
        ...
        // Gather Data Back
        const size_t dims[3]={imsize[0], imsize[1], imsize[2]};
        //cdims = (size_t)imsize;
        ...
        CUDA_SAFE_CALL(cudaMemcpy(o1,d_i2,sizeof(float)*imsize[0]*imsize[1]*imsize[2],cudaMemcpyDeviceToHost));
