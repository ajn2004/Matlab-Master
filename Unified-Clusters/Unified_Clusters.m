...
clc
mkdir('Clust');
perc_nn = 0.8;  % what precentage of the average nearest neighbor distance of nodes to use for circle radius in determining member localizations
min_num = 8; % minimum number of overlapping localizations to be considered apart of the same cluster
...
for i = 1 : numel(w(:,1))
    ...
    ds = ((xf_all.*q - w(i,1)).^2 + (yf_all.*q - w(i,2)).^2).^0.5; % determine distances from each point
...
s = scatter(xf_all*q,yf_all*q,10,clus_id);
...
save([fpath,'Clust\',fname(1:end-4),'_clust.mat']);
% movie2gif(M,'new_cluster.gif','DelayTime',0.03);