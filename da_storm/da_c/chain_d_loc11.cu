...
// localize 11 *updating for 3D 2/7/18
__global__ void  localize11(float *d_iall,  // pointer to our image variable
	...
	int numi)// pointer for final x-coordinate measurement
{
	...
	if (index < numi) {			// check to see that threads only work if an image exists
		...
		if (xf == xf && yf == yf && zf == zf && N == N && b == b && sx == sx && sy == sy && b == b) { // begin is numeric if statement
			if (N > 0 && xf >= -4 && xf <= 4 && yf <= 4 && yf >= -4) { // was the molecule inside the image? Was N positive? if yes then record the point
...
{	
	...
	threadsperblock = mxGetScalar(prhs[2]);  // get number of threads perblock from matlab
		...
		localize11 << <((numi - 1) / threadsperblock + 1), threadsperblock >> >(d_iall, d_xf_all, d_yf_all, d_zf_all, d_N, d_off, d_xf_crlb, d_yf_crlb, d_zf_crlb, d_N_crlb, d_off_crlb, d_llv, angle, d_zcurve, numi);
